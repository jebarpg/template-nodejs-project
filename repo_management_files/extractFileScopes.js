...
function removeAbsolutePathToLowerCase(mainString) {
  ...
  const re = new RegExp(
    '(?:' + escapedQuery + '){1,' + MAX_QUERY_LENGTH + '}',
    'ig'
  );
        ...
        return new Promise((resolve, reject) => {
          ...
          stat(filePath, (err, stats) => {
            ...
            if (stats.isDirectory()) {
              ...
              listFilesAndDirectories(filePath, excludeList)
                .then((innerFileScopes) => {
...
listFilesAndDirectories(directoryPath, exclude)
  .then((fileScopes) => {
    updateScopesInPackageJson(
      directoryPath + 'package.json',
      fileScopes.map(removeAbsolutePathToLowerCase)
    );
  })
  .catch((err) => {
